...
const mockBackendFetch = backendFetch as jest.MockedFunction<
  typeof backendFetch
>;
...
describe("EditAvatarModal", () => {
  ...
  it("shows error for non-image file", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Solo se permiten archivos de imagen"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("successful upload calls backendFetch with FormData", async () => {
    ...
    const uploadRes = {
      ...
      json: jest
        .fn()
        .mockResolvedValue({ avatarUrl: "https://example.com/avatar.jpg" }),
    ...
    await waitFor(() => {
      expect(
        screen.getByAltText("Vista previa del avatar"),
      ).toBeInTheDocument();
    });
  ...
  it("shows error when upload fails", async () => {
    ...
    const errorRes = {
      ...
      json: jest
        .fn()
        .mockResolvedValue({ message: "No se pudo actualizar el avatar" }),
    } as unknown as Response;
    mockBackendFetch.mockResolvedValueOnce(errorRes);
    mockExtractErrorMessage.mockResolvedValueOnce(
    ...
    await waitFor(() => {
      expect(
        screen.getByText("No se pudo actualizar el avatar"),
      ).toBeInTheDocument();
    });
  });
