...
import Channel from "../../src/services/channel/Channel.model.js";
import Server from "../../src/services/server/Server.model.js";
import {
  resetDatabase,
  startE2EServer,
  stopE2EServer,
    ...
    const msgPromise = new Promise((resolve, reject) => {
      clientB.once("message", async (msg) => {
        try {
          ...
          expect(senderId.toString()).toBe(userId.toString());

          // Verificar que realmente se guardó en DB
          const stored = await mongoose.connection
            .collection("messages")
            .findOne({ _id: new mongoose.Types.ObjectId(msg.id) });
