...
describe("/api/ServerInvites E2E", () => {
  test("permite enviar y aceptar una invitación de servidor", async () => {
    ...
    const pendingRes = await request(app)
      .get("/api/server-invites/pending")
    ...
    const acceptRes = await request(app)
      .post(`/api/server-invites/accept/${invite.id}`)
      .set(authHeader(invitedToken));
  ...
  test("rechazar invitación no agrega al servidor", async () => {
    ...
    const rejectRes = await request(app)
      .post(`/api/server-invites/reject/${invite.id}`)
      .set(authHeader(invitedToken));
  ...
  test("responder invitación inexistente devuelve 404", async () => {
    ...
    const res = await request(app)
      .post(`/api/server-invites/accept/${new Types.ObjectId()}`)
      .set(authHeader(token));
  ...
  test("requiere autenticación para enviar invitaciones", async () => {
    ...
    const resNoToken = await request(app).post("/api/server-invites/send").send(payload);
  ...
  test("requiere autenticación para listar invitaciones pendientes", async () => {
    const resNoToken = await request(app).get("/api/server-invites/pending");
    expectFail(resNoToken, 401, AUTH_ERROR.message, AUTH_ERROR.code);
