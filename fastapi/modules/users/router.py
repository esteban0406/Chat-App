...
async def search_users(username: Annotated[str, Query(...)], db: DbDep):
...
async def update_me(dto: UpdateUserDTO, current_user: CurrentUserDep, db: DbDep):
    return await service.update(db, current_user, dto)


@router.patch("/me/status", response_model=UserResponse)
async def update_status(dto: UpdateStatusDTO, current_user: CurrentUserDep, db: DbDep):
